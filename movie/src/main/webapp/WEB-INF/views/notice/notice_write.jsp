			...
			<form action="saveNotice" method="post">
			  ...
			  <div class="form-group">
			   <label for="title">게시글번호</label>
			    <input type="text" class="form-control" id="noticeTitle" value="${notice.noticeTitle }" readonly="readonly">
			    <label for="title">제목</label>
			    <input type="text" class="form-control" id="noticeTitle" name="noticeTitle"  value="${notice.noticeTitle }">
			    <label for="title">구분</label>
			    <select class="form-control" name="noticeClass" id="noticeClass">
			  ...
			  <div class="form-group">
			    ...
			    <textarea class="form-control" id="exampleFormControlTextarea1" name="noticeContent"  rows="10" >${notice.noticeContent}</textarea>
			  </div>
				<div class="form-group text-center">
					<a class="btn btn-primary" href="notice">뒤로가기</a>
			 		 <button type="submit" class="btn btn-primary">저장</button>			  	
			  		<c:if test="${notice.noticeNo !=null}">
			  		 <button type="button" class="btn btn-primary" onclick="deleteNotice(${notice.noticeNo})">삭제</button>		
			  		</c:if>
